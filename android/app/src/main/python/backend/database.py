...
# ---------------------------------------------------------------------------
# 部分平台（如部分 Android 构建）SQLite 未编译 FTS5，此时回退到 LIKE 搜索
...
async def init_db() -> None:
    ...
    async with _engine.begin() as conn:
        ...
        try:
            ...
            # SQLite 未编译 FTS5 时回退到 LIKE 搜索，保证功能可用
            _fts5_available = False
            logging.getLogger("weibo.db").warning("FTS5 不可用，搜索将回退到 LIKE：%s", e)
...
async def fts_search(db: AsyncSession, keyword: str, page: int = 1, page_size: int = 20) -> tuple:
    ...
    if not _fts5_available:
        # FTS5 不可用：回退到 LIKE 模糊搜索
        like = f"%{keyword}%"
        total = await db.scalar(
            select(func.count(Post.id)).where(Post.is_own == True, Post.text.like(like))  # noqa: E712
        )
        result = await db.execute(
            select(Post).where(Post.is_own == True, Post.text.like(like))  # noqa: E712
            .order_by(Post.created_at.desc()).limit(page_size).offset(offset)
        )
        return list(result.scalars().all()), total or 0
