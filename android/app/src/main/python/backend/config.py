...
def is_android() -> bool:
    """是否运行在 Android（Chaquopy）环境。"""
    if sys.platform == "android":
        return True
    if os.environ.get("ANDROID_PRIVATE"):
        return True
    # Chaquopy 将 sys.platform 报告为 "linux"，但会提供 java 模块
    try:
...
def _android_home() -> Path:
    """Android 应用内部存储目录（Chaquopy 将 HOME 指向 getFilesDir）。"""
    return Path(os.environ.get("HOME") or os.environ.get("ANDROID_PRIVATE") or "/data/data/com.weiboarchive/files")
...
def _settings_file() -> Path:
    ...
    - Android 版：应用内部存储 WeiboArchive/settings.json
    ...
    if is_android():
        return _android_home() / "WeiboArchive" / "settings.json"
...
def _data_root() -> Path:
    ...
    - Android 版：默认应用内部存储 WeiboArchive/workspace
    ...
    if is_android():
        return _android_home() / "WeiboArchive" / "workspace"
