...
def _extract_config(html: str) -> dict:
    """从 weibo.com 首页 HTML 提取 window.$CONFIG 对象。

    用括号配对 + 字符串状态机解析，避免正则非贪婪匹配在嵌套对象处提前截断。
    """
    m = re.search(r"window\.\$CONFIG\s*=\s*(\{)", html)
    if not m:
        return {}
    start = m.start(1)
    depth = 0
...
def _extract_cookie_from_headers(headers, name: str) -> str:
    """从响应 Set-Cookie 头中提取指定 Cookie 的值（httpx Headers 对象）。"""
    try:
...
class WeiboClient:
    ...
    async def refresh_token(self) -> Optional[str]:
        ...
        try:
            # 用 weibo.com 首页验证登录态并刷新 _T_WM。
            # 注意：m.weibo.cn/api/config 对部分 WebView 登录的 Cookie 会返回 login:false，
            # 而 weibo.com 首页 $CONFIG 能正确识别登录态，故统一走 weibo.com。
            self._cookies["_T_WM"] = int(time.time() / 3600) * 100001
            resp = await self._raw_get("https://weibo.com/", referer="https://weibo.com/")
            resp.raise_for_status()
            cfg = _extract_config(resp.text)
            uid = cfg.get("uid") or (cfg.get("user") or {}).get("id")
            if not uid:
                raise WeiboAuthError("Cookie 无效或未登录")
            # weibo.com 首页每次会通过 Set-Cookie 下发新的 XSRF-TOKEN，捕获并保存
            new_token = _extract_cookie_from_headers(resp.headers, "XSRF-TOKEN")
            if new_token:
                self._cookies["XSRF-TOKEN"] = new_token
            token = self._cookies.get("XSRF-TOKEN") or ""
            if token:
                self._save_cookies()
                logger.info("XSRF-TOKEN 校验通过。")
...
async def resolve_container_id(client: WeiboClient) -> tuple[int, str]:
    """通过 weibo.com 获取 uid 与 containerid（containerid 仅作兼容保留）。"""
    uid, _ = await resolve_self_profile(client)
    cid = ""
    try:
        ...
        cid = str(user.get("containerid", "") or "").split("?")[0]
    except Exception:
        pass
    return uid, cid
...
async def resolve_self_profile(client: WeiboClient) -> tuple[int, str]:
    """通过 weibo.com 首页 $CONFIG 获取登录账号的 uid 与昵称。

    m.weibo.cn/api/config 对部分 WebView 登录的 Cookie 会返回 login:false，
    而 weibo.com 首页能正确识别登录态，故统一走 weibo.com。
    """
    ...
    if not uid:
        raise WeiboAuthError("Cookie 无效或未登录")
    name = (cfg.get("user") or {}).get("screen_name", "") or ""
    return int(uid), name