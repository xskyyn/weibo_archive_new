...
- **历史视频缺失**：修复旧版视频博文（`page_info.type="11"`，2022-2026 年）无法解析出视频媒体的问题。解析器兼容 `object_type=="video"` 并优先选择 720p 清晰度；已回填历史视频并下载完整
- **仅归档原创视频**：转发微博的视频属于原博主，解析器不再收录（`retweeted_status_id` 非空的转发微博跳过视频解析），只保留自己发布的视频
- **前端部署**：前端构建产物 `frontend/dist` 不纳入 git，服务器部署需单独构建或上传 dist（附 `dist/webo-frontend-dist-1.6.0.zip` 供服务器直接解压使用）
...
- **归档浏览分页**：新增每页条数选择（10 / 20 / 50 / 自定义输入，上限 200），底部标准翻页组件（总数、页码、跳页），替代原无限滚动加载
- **媒体时光轴排序**：新增正序/倒序切换（`/api/media` 支持 `order=asc|desc`），倒序为最新在前（默认），正序为最早在前
- **往年今日筛选**：归档浏览新增「往年今日」按钮，一键展示同月同日、往年发布的微博（排除今年），激活时自动禁用关键词/年份/月份筛选
