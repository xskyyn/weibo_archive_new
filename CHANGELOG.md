...
- **断点续抓失效**：归档循环以「本页新增 0 条」作为终止条件，导致重跑时第 1 页（最新微博均已归档）就提前 break，永远到不了缺失的旧微博页（实测重装后仍停在 1068 条）。已移除该终止条件，改为仅依赖接口返回空列表判定归档结束，并把已归档的本人微博 mid 载入 `seen_ids` 实现断点续抓
...
- **归档任务崩溃**：`task_manager.py` 在 `except WeiboAuthError` 中使用了未导入的异常类，导致微博接口返回认证错误（`http_code=400`，账号受限或 Cookie 失效）时任务以 `NameError` 崩溃、归档中断（实测停在 1068 条）。已补全 `WeiboAuthError` 导入，认证错误将按预期以清晰报错结束任务
